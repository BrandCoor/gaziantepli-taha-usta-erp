...
export const PosView: React.FC<PosViewProps> = ({ autoOpenTableId, onClearAutoOpen }) => {
  ...
  const [productCardViewMode, setProductCardViewMode] = useState<'PHOTO' | 'COMPACT'>('PHOTO');
  ...
  const filteredTables = sortTablesNaturally((tables || []).filter(t => t.sectionId === selectedSectionId), sections);
            ...
            <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden">
              ...
              <div className={`flex-1 flex-col h-full bg-[#1C1C20] overflow-hidden ${
                ...
                <div className="p-3 sm:p-4 border-b border-[#2C2C34] space-y-2.5 bg-[#1C1C20] flex-shrink-0">
                  ...
                  {/* Yatay Kategori Çipleri ve Görünüm Modu */}
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 overflow-x-auto pb-1 hide-scrollbar flex-1">
                      <button
                    ...
                    {/* Görünüm Modu Seçici (Fotoğraflı / Sade Liste) */}
                    <div className="flex items-center bg-[#141416] p-0.5 rounded-xl border border-[#2C2C34] shrink-0">
                      <button
                        type="button"
                        onClick={() => setProductCardViewMode('PHOTO')}
                        className={`p-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 ${
                          productCardViewMode === 'PHOTO'
                            ? 'bg-[#F5C877] text-[#141416] shadow-sm'
                            : 'text-[#8E8E98] hover:text-white'
                        }`}
                        title="Fotoğraflı Kartlar"
                      >
                        <LayoutGrid className="w-3.5 h-3.5" />
                        <span className="hidden lg:inline text-[10px]">Fotoğraflı</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => setProductCardViewMode('COMPACT')}
                        className={`p-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 ${
                          productCardViewMode === 'COMPACT'
                            ? 'bg-[#F5C877] text-[#141416] shadow-sm'
                            : 'text-[#8E8E98] hover:text-white'
                        }`}
                        title="Kompakt Sade Liste"
                      >
                        <List className="w-3.5 h-3.5" />
                        <span className="hidden lg:inline text-[10px]">Kompakt</span>
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 p-3 sm:p-4 overflow-y-auto">
                  {filteredProducts.length === 0 ? (
                    ...
                    <div className={
                      productCardViewMode === 'PHOTO'
                        ? "grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3"
                        : "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2.5"
                    }>
                      {filteredProducts.map((prod) => {
                        ...
                        if (productCardViewMode === 'PHOTO') {
                          return (
                            <button
                              key={prod.id}
                              onClick={() => handleAddProduct(prod)}
                              className={`rounded-2xl text-left transition-all group flex flex-col justify-between cursor-pointer relative border overflow-hidden shadow-sm ${
                                inCartCount > 0
                                  ? 'bg-[#1F1E1C] border-[#F5C877] shadow-md shadow-[#F5C877]/10 ring-1 ring-[#F5C877]/40'
                                  : 'bg-[#141416] hover:bg-[#1E1E24] border-[#2C2C34] hover:border-[#F5C877]/60'
                              } active:scale-95`}
                            >
                              {/* Ürün Fotoğrafı */}
                              <div className="relative w-full h-28 sm:h-32 bg-[#1C1C20] overflow-hidden">
                                <img 
                                  src={displayImg} 
                                  alt={prod.name}
                                ...
                                {inCartCount > 0 && (
                                  <span className="absolute top-2 right-2 px-2 py-0.5 bg-[#F5C877] text-[#141416] rounded-lg text-[10px] font-black shadow-lg">
                                    {inCartCount} Adet
                                  </span>
                                )}

                                <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between text-[10px] text-[#FAF7F2] font-bold">
                                  <span className="bg-black/60 backdrop-blur-sm px-1.5 py-0.5 rounded-md text-[9px] text-[#A0A0AA]">
                                    ~{prod.preparationMin || 15} dk
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="p-2.5 flex-1 flex flex-col justify-between w-full">
                                <div className="font-black text-xs text-[#FAF7F2] group-hover:text-[#F5C877] line-clamp-1">
                                  {prod.name}
                                </div>

                                <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#2C2C34]/60">
                                  <span className="font-black text-sm text-[#F5C877] font-mono">
                                    {prod.price} ₺
                                  </span>
                                  <div className="w-6 h-6 rounded-xl bg-[#F5C877] text-[#141416] flex items-center justify-center font-black group-hover:scale-110 transition-transform shadow-md">
                ...
                <div className="flex-1 p-3 overflow-y-auto space-y-2">
                  {orderItems.length === 0 ? (
                    ...
                    orderItems.map((item, idx) => {
                      ...
                      const prodObj = products.find(p => p.id === item.productId || p.name === item.productName);
                      const itemImg = (prodObj && prodObj.imageUrl) || getProductFallbackImage(item.productName, prodObj?.categoryId);
                      ...
                      return (
                        ...
                        >
                          {/* 1. Satır: Görsel, Ürün Adı, Rozet, Adet Kontrolü (+ / -) ve Satır Tutarı */}
                          <div className="flex items-center justify-between gap-2.5">
                            <div className="flex items-center gap-2.5 min-w-0 flex-1">
                              <div className="w-10 h-10 rounded-xl bg-[#141416] border border-[#2C2C34] overflow-hidden shrink-0 shadow-sm">
                                <img 
                                  src={itemImg} 
                                  alt={item.productName} 
                                  className="w-full h-full object-cover"
                                  onError={(e) => {
                                    (e.target as HTMLImageElement).src = getProductFallbackImage(item.productName, prodObj?.categoryId);
                                  }}
                                />
                              </div>
