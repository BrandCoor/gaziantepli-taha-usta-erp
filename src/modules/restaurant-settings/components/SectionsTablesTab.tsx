...
import { SectionConfig, restaurantDataService, getPublicBaseUrl } from '../../../services/restaurantDataService';
...
export const SectionsTablesTab: React.FC<SectionsTablesTabProps> = ({ sections, onRefresh }) => {
  ...
  const [selectedQrTable, setSelectedQrTable] = useState<{ id: string; name: string; sectionName: string } | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#1C1C20] p-5 rounded-3xl border border-[#2C2C34] shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-3.5 py-2.5 bg-[#2C2C34] hover:bg-[#383844] text-white text-xs font-bold rounded-xl flex items-center gap-1.5 shadow-md cursor-pointer transition-all border border-[#3E3E4A]"
            title="Masa QR Kodları ve Dijital Menü Çıktısı Al"
          >
            <QrCode className="w-4 h-4 text-[#F5C877]" />
            <span>QR Menü & Masa Kodları</span>
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {sections.map((sec) => (
          <div key={sec.id} className="bg-[#1C1C20] rounded-3xl p-5 border border-[#2C2C34] shadow-lg space-y-4 hover:border-[#3E3E4A] transition-all flex flex-col justify-between">
            ...
            <div className="pt-2 text-[11px] text-[#A0A0AA] flex items-center justify-between">
              <div className="flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                <span>Adisyon & POS aktif</span>
              </div>
              <button
                onClick={() => {
                  setSelectedQrTable({ id: `${sec.id}-1`, name: `${sec.name} - Masa 1`, sectionName: sec.name });
                  setQrModalOpen(true);
                }}
                className="text-xs text-[#F5C877] hover:underline flex items-center gap-1 font-bold cursor-pointer"
              >
                <QrCode className="w-3.5 h-3.5" />
                <span>QR Kodları Gör</span>
              </button>
            </div>
          </div>
        ))}
      ...
      {/* QR MENÜ & MASA KODLARI MODALI */}
      {qrModalOpen && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center p-4 z-50 backdrop-blur-md animate-fadeIn">
          <div className="bg-[#141416] rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-[#2C2C34] space-y-5 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between border-b border-[#2C2C34] pb-3 shrink-0">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl bg-[#F5C877]/10 text-[#F5C877] border border-[#F5C877]/30 flex items-center justify-center">
                  <QrCode className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-sm font-black text-white">Masa QR Menü & Sipariş Kartı</h3>
                  <p className="text-[11px] text-[#A0A0AA]">Müşteriler kamerayla okutup direkt bu masaya sipariş verebilir</p>
                </div>
              </div>
              <button onClick={() => setQrModalOpen(false)} className="text-[#A0A0AA] hover:text-white text-xs font-bold cursor-pointer p-1">✕ Kapat</button>
            </div>

            <div className="overflow-y-auto space-y-4 pr-1">
              {/* Masa Seçici */}
              <div className="bg-[#1C1C20] p-4 rounded-2xl border border-[#2C2C34] space-y-2">
                <label className="text-xs font-bold text-[#C4C4CC] block">Masa Seçimi:</label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 max-h-36 overflow-y-auto p-1">
                  {sections.flatMap(sec => 
                    Array.from({ length: sec.tableCount }, (_, i) => {
                      const tableNum = i + 1;
                      const tableName = `${sec.name} Masa ${tableNum}`;
                      const tableId = `${sec.id}-${tableNum}`;
                      const isSelected = selectedQrTable ? selectedQrTable.id === tableId : tableNum === 1;
                      return (
                        <button
                          key={tableId}
                          onClick={() => setSelectedQrTable({ id: tableId, name: tableName, sectionName: sec.name })}
                          className={`p-2 rounded-xl text-left text-xs font-bold border transition-all cursor-pointer ${
                            isSelected 
                              ? 'bg-[#F5C877] text-slate-950 border-[#F5C877] font-black' 
                              : 'bg-[#141416] text-[#C4C4CC] border-[#2C2C34] hover:border-slate-600'
                          }`}
                        >
              ...
              {/* QR Önizleme & Link Alanı */}
              {(() => {
                const activeTable = selectedQrTable || { 
                  id: `${sections[0]?.id || 'sec'}-1`, 
                  name: `${sections[0]?.name || 'Salon'} Masa 1`,
                  sectionName: sections[0]?.name || 'Salon'
                };
                const publicBase = getPublicBaseUrl();
                const hostOrigin = typeof window !== 'undefined' ? (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? publicBase : window.location.origin) : publicBase;
                const tableParam = activeTable.name.replace(/\D/g, '') || activeTable.id;
                const qrUrl = `${hostOrigin.replace(/\/+$/, '')}/menu/?masa=${tableParam}`;
                const qrImageSrc = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(qrUrl)}&margin=8`;

                return (
                  <div className="bg-gradient-to-b from-[#1C1C20] to-[#161619] p-6 rounded-3xl border border-[#F5C877]/30 text-center space-y-4">
                    <div className="inline-block p-3 bg-white rounded-2xl shadow-xl border-4 border-[#F5C877]">
                      <img 
                        src={qrImageSrc} 
                        alt="Masa QR Kod" 
                    ...
                    <div>
                      <h4 className="text-base font-black text-white">{activeTable.name}</h4>
                      <p className="text-xs text-amber-300 font-mono mt-0.5">Gaziantepli Taha Usta • Dijital QR Menü</p>
                    </div>

                    <div className="p-3 bg-[#141416] rounded-xl border border-[#2C2C34] flex items-center justify-between gap-2 text-left">
                      <div className="truncate text-xs font-mono text-emerald-400">
                        {qrUrl}
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          onClick={() => {
                            ...
                            notify.success('Kopyalandı', 'Masa QR linki panoya kopyalandı.');
                          }}
                          className="p-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-bold flex items-center gap-1 cursor-pointer"
                          title="Linki Kopyala"
                        >
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="p-2 bg-[#F5C877] hover:bg-[#e4b764] text-slate-950 rounded-lg text-xs font-bold flex items-center gap-1 cursor-pointer"
                    ...
                    <div className="pt-2 flex items-center justify-center gap-3">
                      <button
                        ...
                        className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-black rounded-xl flex items-center gap-1.5 cursor-pointer border border-[#3E3E4A]"
                      >
                        <Printer className="w-4 h-4 text-[#F5C877]" />
                        <span>Masa QR Kartını Yazdır</span>
                      </button>
                    </div>
                  </div>
                );
