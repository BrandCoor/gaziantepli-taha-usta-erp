...
import { PrinterConfig, restaurantDataService, getApiSyncUrl } from '../../../services/restaurantDataService';
...
export const PrintersTab: React.FC<PrintersTabProps> = ({ printers, onRefresh }) => {
  ...
  const handleFullHardwareScan = async () => {
    ...
    // 1.5 Yerel POS Sunucusundan (port 4545) Windows Spooler Yazıcılarını Çek
    try {
      const winResp = await fetch('http://localhost:4545/api/printers/windows-printers');
      if (winResp.ok) {
        const winData = await winResp.json();
        if (winData.success && Array.isArray(winData.printers)) {
          winData.printers.forEach((wp: any) => {
            if (!foundUsb.some((u: any) => u.displayName === wp.displayName || (u.name && u.name === wp.name))) {
              foundUsb.push(wp);
            }
          });
        }
  ...
  const openEditPrinterModal = (pr: PrinterConfig) => {
    ...
    setForm({
      ...
      failoverPrinterId: pr.failoverPrinterId || '',
    });
    setModalOpen(true);
  };
  ...
  const handleSavePrinterSubmit = (e: React.FormEvent) => {
    ...
    // Doğrudan MySQL / phpMyAdmin API'ye aktar
    fetch(`${getApiSyncUrl()}?action=save_printer`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: targetId, ...form })
    }).catch(() => {});
  ...
  const handleHardwareTestPrint = async (pr: PrinterConfig) => {
    restaurantDataService.playAudioAlert('beep');
    const res = await printerService.printHardwareTest(pr);
    if (res.success) {
      notify.success('Donanım Testi Başarılı', `[${pr.name}] ESC/POS ve Win32 Spooler donanım test çıktısı döküldü. Para çekmecesi ve kağıt kesici tetiklendi.`);
    } else {
      notify.warning('Donanım Çıktısı', `[${pr.name}] Test komutları spooler kuyruğuna iletildi: ${res.message || 'Sistem yanıt verdi'}`);
    }
  };
              ...
              <div key={pr.id} className="bg-[#1C1C20] p-5 rounded-3xl border border-[#2C2C34] shadow-lg flex flex-col justify-between space-y-4 hover:border-[#3E3E4A] transition-all">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <button
                    ...
                    className="py-2.5 px-2 bg-[#141416] hover:bg-slate-800 border border-[#383844] text-white text-[11px] font-black rounded-xl flex items-center justify-center gap-1.5 cursor-pointer transition-all hover:border-[#F5C877]"
                    title="Ağ veya USB üzerinden standart test fişi gönderir"
                  >
                    <Printer className="w-3.5 h-3.5 text-[#F5C877]" />
                    <span>Test Fişi & Bip</span>
                  </button>

                  <button
                    onClick={() => handleHardwareTestPrint(pr)}
                    className="py-2.5 px-2 bg-[#1C1C22] hover:bg-amber-950/40 border border-amber-500/40 text-amber-300 text-[11px] font-black rounded-xl flex items-center justify-center gap-1.5 cursor-pointer transition-all hover:border-amber-400"
                    title="Afanda 892E ve Win32 RAW Spooler, ESC/POS CP857 Türkçe, Para Çekmecesi ve Kağıt Kesim testi"
                  >
                    <Zap className="w-3.5 h-3.5 text-amber-400" />
                    <span>Donanım & Kesici Testi</span>
                  </button>
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50 backdrop-blur-md animate-fadeIn">
          <div className="bg-[#141416] rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-[#2C2C34] space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSavePrinterSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="USB">USB Kablo (Doğrudan Port)</option>
                    <option value="SYSTEM_DRIVER">Windows Sürücüsü / Spooler</option>
              ...
              ) : form.type === 'SYSTEM_DRIVER' ? (
                <div className="p-3 bg-[#1C1C20] rounded-2xl border border-[#2C2C34]">
                  <label className="text-[11px] font-bold text-[#C4C4CC]">Windows Yazıcı Sürücü Adı (Spooler)</label>
                  <input
                    type="text"
                    value={form.driverName}
                    onChange={(e) => setForm({ ...form, driverName: e.target.value })}
                    placeholder="Örn: POS-80, EPSON TM-T20III veya Generic / Text Only"
                    className="w-full mt-1 p-2 bg-[#141416] border border-[#383844] rounded-xl text-xs font-bold text-white focus:outline-none"
                  />
                </div>
              ) : form.type === 'USB' ? (
                <div className="p-3 bg-[#1C1C20] rounded-2xl border border-[#2C2C34]">
                  <label className="text-[11px] font-bold text-[#C4C4CC]">USB Aygıt / Model Adı</label>
                  <input
                    type="text"
                    value={form.usbName}
              ...
              {/* OTOMATİK YEDEK (FAILOVER) YAZICI TANIMI */}
              <div className="p-3 bg-[#1C1C20] rounded-2xl border border-[#2C2C34]">
                <div className="flex items-center justify-between mb-1">
                  <label className="text-[11px] font-bold text-[#C4C4CC] flex items-center gap-1.5">
                    <ShieldCheck className="w-3.5 h-3.5 text-amber-400" />
                    Otomatik Yedek (Failover) Yazıcı
                  </label>
                  <span className="text-[10px] text-zinc-400 font-medium">Bağlantı koparsa veya kağıt biterse devreye girer</span>
                </div>
                <select
                  value={form.failoverPrinterId || ''}
                  onChange={(e) => setForm({ ...form, failoverPrinterId: e.target.value || undefined })}
                  className="w-full mt-1 p-2 bg-[#141416] border border-[#383844] rounded-xl text-xs font-bold text-white focus:outline-none"
                >
                  <option value="">-- Yedek Yazıcı Yok (Aktarma Yapma) --</option>
                  {printers.filter(p => p.id !== editingId).map(p => (
                    <option key={p.id} value={p.id}>{p.name} ({p.type === 'NETWORK' ? p.ipAddress : p.type})</option>
                  ))}
                </select>
              </div>
