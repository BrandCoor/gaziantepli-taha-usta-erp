        ...
        <div className="bg-[#1C1C20] rounded-3xl p-6 border border-[#2C2C34] shadow-xl flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between border-b border-[#2C2C34] pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-white">GCallerID 2Hat & Telefon Entegrasyonu</h3>
                  <p className="text-xs text-[#C4C4CC]">Gentema GCallerID 2Hat USB/COM ve HTTP Webhook canlı arayan tespiti</p>
                </div>
              </div>
            ...
            <div className="mt-4 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-[#C4C4CC]">Cihaz Donanım Tipi</label>
                  <select
                    ...
                    className="w-full mt-1 p-2.5 bg-[#141416] border border-[#383844] rounded-xl text-xs font-bold text-white focus:outline-none disabled:opacity-50"
                  >
                    <option value="GCALLERID_2HAT">GCallerID 2Hat (Gentema USB)</option>
                    ...
                    <option value="GENERIC_MODEM">Standart Seri COM Port</option>
                  </select>
                </div>

                <div>
                  <div className="flex items-center justify-between">
                    <label className="text-[11px] font-bold text-[#C4C4CC]">COM Portu</label>
                    <button
                      type="button"
                      onClick={async () => {
                        const res = await callerIdService.connectPort('AUTO');
                        notify.info('Port Algılama', res.message || 'Otomatik port tarandı.');
                      }}
                  ...
                  <select
                    ...
                    className="w-full mt-1 p-2.5 bg-[#141416] border border-[#383844] rounded-xl text-xs font-bold text-white focus:outline-none disabled:opacity-50"
                  >
                    <option value="AUTO">AUTO (Otomatik COM Tespiti)</option>
                    <option value="COM1">COM1</option>
                    <option value="COM2">COM2</option>
                    <option value="COM3">COM3</option>
              ...
              <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800 space-y-1.5">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="font-bold text-slate-300">GCallerID HTTP Webhook URL:</span>
                  <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-800/60">
                    Aktif (Port 4545)
                  </span>
                </div>
                <div className="p-1.5 bg-black/40 rounded border border-slate-700/50 font-mono text-[10px] text-amber-300 select-all">
                  http://127.0.0.1:4545/api/callerid?hat=1&telefon=[TEL]
                </div>
                <p className="text-[10px] text-slate-400">
                  GCallerID Windows yazılımının &quot;URL Aç&quot; özelliğinde bu adresi tanımlayarak da çağrıları anında yakalayabilirsiniz.
                </p>
              </div>

              <div className="p-3 bg-[#141416] rounded-2xl border border-[#2C2C34] space-y-2">
                <label className="flex items-center justify-between cursor-pointer">
                  <span className="text-[#C4C4CC] font-bold">Arama geldiğinde otomatik paket adisyonu öner</span>
                  <input
                    type="checkbox"
                    disabled={!config.callerId.enabled}
                ...
                <label className="flex items-center justify-between cursor-pointer">
                  <span className="text-[#C4C4CC] font-bold">Eşzamanlı 2 Hat pop-up bildirim kartlarını göster</span>
                  <input
                    type="checkbox"
                    disabled={!config.callerId.enabled}
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              disabled={!config.callerId.enabled}
              onClick={() => {
                callerIdService.simulateCall(1, '05321002030');
                notify.info('Hat 1 Çağrı Testi', '🟢 HAT 1 gelen arama bildirimi tetiklendi.');
              }}
              className="py-2.5 bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/40 text-emerald-400 text-xs font-black rounded-xl flex items-center justify-center gap-1.5 transition-all disabled:opacity-50"
            >
              <span>🟢 Hat 1 Test Et</span>
            </button>
            ...
            <button
              ...
              disabled={!config.callerId.enabled}
              onClick={() => {
                callerIdService.simulateCall(2, '05422003040');
                notify.info('Hat 2 Çağrı Testi', '🔵 HAT 2 gelen arama bildirimi tetiklendi.');
              }}
              className="py-2.5 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/40 text-blue-400 text-xs font-black rounded-xl flex items-center justify-center gap-1.5 transition-all disabled:opacity-50"
            >
              <span>🔵 Hat 2 Test Et</span>
            </button>
          </div>
        </div>
