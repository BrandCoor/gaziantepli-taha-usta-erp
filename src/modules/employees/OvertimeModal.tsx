...
export const OvertimeModal: React.FC<OvertimeModalProps> = ({
  ...
  // Saatlik Ücret Hesabı (Maaş tipine göre dinamik: Aylık/26 gün, Haftalık/6 gün, Günlük yevmiye/1 gün)
  const hourlyRate = useMemo(() => {
    if (!selectedEmployee || !selectedEmployee.salary) return 0;
    const dailyHours = Number(normalDailyHours) || 8;
    if (dailyHours <= 0) return 0;

    const salaryType = selectedEmployee.salaryType || 'MONTHLY';
    if (salaryType === 'DAILY') {
      return Math.round((Number(selectedEmployee.salary) / dailyHours) * 100) / 100;
    } else if (salaryType === 'WEEKLY') {
      const weeklyHours = dailyHours * 6; // Haftada 6 çalışma günü
      return Math.round((Number(selectedEmployee.salary) / weeklyHours) * 100) / 100;
    } else {
      const monthlyHours = dailyHours * 26; // Ayda 26 çalışma günü
            ...
            <div>
              ...
              >
                {employees.map((emp) => {
                  ...
                  const sTypeLabel = sType === 'DAILY' ? 'Günlük Yevmiye' : sType === 'WEEKLY' ? 'Haftalık' : 'Aylık';
                  return (
                    <option key={emp.id} value={emp.id}>
                      {emp.fullName} ({emp.position || 'Personel'}) - {emp.salary.toLocaleString('tr-TR')} ₺ / {sTypeLabel}
                    </option>
                  );
                })}
              </select>
                ...
                <div className="mt-1.5 flex items-center justify-between text-[11px] text-[#A0A0AA] px-1">
                  <span>
                    ...
                    <strong className="text-emerald-400">
                      {selectedEmployee.salary.toLocaleString('tr-TR')} ₺
                    </strong>
                  </span>
