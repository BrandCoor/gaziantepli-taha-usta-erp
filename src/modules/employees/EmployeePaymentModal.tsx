...
import { Employee, dataService, WEEKDAY_LABELS } from '../../services/dataService';
...
export const EmployeePaymentModal: React.FC<EmployeePaymentModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const sType = selectedEmployee?.salaryType || 'MONTHLY';

    let defaultDesc = '';
    if (type === 'ADVANCE') {
      defaultDesc = 'Maaş Avansı Verildi';
    } else if (type === 'SALARY_PAYMENT') {
      defaultDesc = sType === 'DAILY' ? 'Günlük Yevmiye Ödemesi' : sType === 'WEEKLY' ? 'Haftalık Maaş Ödemesi' : 'Aylık Maaş / Hakediş Ödemesi';
    } else if (type === 'BONUS') {
      defaultDesc = 'Prim / Bahşiş Hakedişi';
    } else if (type === 'DEDUCTION') {
      defaultDesc = 'Maaş Kesintisi';
    } else if (type === 'SALARY_ACCRUAL') {
      defaultDesc = sType === 'DAILY' ? 'Günlük Yevmiye Hakediş Tahakkuku' : sType === 'WEEKLY' ? 'Haftalık Maaş Hakediş Tahakkuku' : 'Aylık Maaş Hakediş Tahakkuku';
    }
          ...
          <div>
            ...
            >
              {employees.map((emp) => {
                ...
                const sTypeLabel = sType === 'DAILY' ? 'Günlük' : sType === 'WEEKLY' ? 'Haftalık' : 'Aylık';
                return (
                  <option key={emp.id} value={emp.id}>
                    {emp.fullName} ({emp.position || 'Personel'}) - {emp.salary.toLocaleString('tr-TR')} ₺/{sTypeLabel} - Bakiye: {emp.balance.toLocaleString('tr-TR')} ₺
                  </option>
                );
              })}
            </select>
            {selectedEmployee && (
              <div className="mt-1.5 flex items-center justify-between text-[11px] text-[#8E8E98] px-1">
                <span>
                  {selectedEmployee.salaryType === 'DAILY'
                    ? 'Günlük Yevmiye: '
                    : selectedEmployee.salaryType === 'WEEKLY'
                    ? 'Haftalık Ücret: '
                    : 'Aylık Maaş: '}
                  <strong className="text-white font-mono">{selectedEmployee.salary.toLocaleString('tr-TR')} ₺</strong>
                  {selectedEmployee.salaryType === 'WEEKLY' && (
                    <span className="text-amber-400 font-bold ml-1">(Her {WEEKDAY_LABELS[selectedEmployee.salaryPaymentDay || 5]})</span>
                  )}
                  {(selectedEmployee.salaryType === 'MONTHLY' || !selectedEmployee.salaryType) && (
                    <span className="text-purple-400 font-bold ml-1">(Ayın {selectedEmployee.salaryPaymentDay || 1}'i)</span>
                  )}
                </span>
                <span>Ödenecek Bakiye: <strong className={`font-mono font-bold ${selectedEmployee.balance > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>{selectedEmployee.balance.toLocaleString('tr-TR')} ₺</strong></span>
              </div>
            )}
          </div>
