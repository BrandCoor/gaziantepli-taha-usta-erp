...
export const EmployeeListView: React.FC<EmployeeListViewProps> = ({ 
  ...
  const handleSaveEmployee = (e: React.FormEvent) => {
    ...
    const finalPaymentDay = formSalaryType === 'DAILY' ? 0 : Number(formSalaryPaymentDay) || 1;
    ...
    if (editingEmployee) {
      dataService.updateEmployee(editingEmployee.id, {
        ...
        salary: salaryNum,
        salaryPaymentDay: finalPaymentDay,
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-[#26262E] text-[#8E8E98] bg-[#141416]">
                  ...
                  <th className="p-3.5 font-black">Ödeme Periyodu & Durum</th>
              ...
              <tbody className="divide-y divide-[#202028]">
                {processedEmployees.length === 0 ? (
                  ...
                  processedEmployees.map((emp) => {
                    ...
                    const divisor = sType === 'DAILY' ? dailyHours : sType === 'WEEKLY' ? (dailyHours * 6) : (dailyHours * 26);
                    const hourlyRate = Math.round(((emp.salary || 0) / (divisor || 1)) * 100) / 100;
                      ...
                      <tr key={emp.id} className="hover:bg-[#18181F] transition-colors">
                        ...
                        <td className="p-3.5 whitespace-nowrap">
                          <div className="flex items-center gap-1.5 font-bold text-white text-xs">
                            ...
                            <span>
                              {sType === 'DAILY' && 'Her Gün (Günlük Yevmiye)'}
                              {sType === 'WEEKLY' && `Her ${WEEKDAY_LABELS[emp.salaryPaymentDay || 5]}`}
                              {sType === 'MONTHLY' && `Ayın ${emp.salaryPaymentDay || 1}. Günü`}
                            </span>
                          </div>
                          <div className="text-[10px] mt-0.5 font-bold">
                            {pendingAccrual ? (
                              <span className="text-amber-400 flex items-center gap-1">
                                ...
                                Tahakkuk Bekliyor ({pendingAccrual.periodName})
                              </span>
                            ) : (
                        ...
                        <td className="p-3.5 text-right font-mono font-bold text-white text-xs">
                          <div>{formatMoney(emp.salary)}</div>
                        ...
                        <td className="p-3.5 text-right font-mono text-amber-300 text-xs">
                          ...
                          <div className="text-[10px] text-[#8E8E98] font-sans">
                            ({sType === 'DAILY' ? `1 gün / ${dailyHours}s` : sType === 'WEEKLY' ? `6 gün / ${dailyHours}s` : `26 gün / ${dailyHours}s`})
                          </div>
                        </td>
        ...
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center p-4 z-50 backdrop-blur-md animate-fadeIn">
          <div className="bg-[#1C1C20] rounded-3xl max-w-md w-full p-6 shadow-2xl border border-[#2C2C34] space-y-4">
            ...
            <form onSubmit={handleSaveEmployee} className="space-y-3.5">
              ...
              {/* Maaş / Ödeme Tipi Seçimi */}
              <div>
                <label className="text-xs font-bold text-[#8E8E98] block mb-1.5">Maaş / Ücret Ödeme Sıklığı</label>
                <div className="grid grid-cols-3 gap-2">
                  <button
                    type="button"
                    onClick={() => {
                      setFormSalaryType('MONTHLY');
                      if (formSalaryPaymentDay < 1 || formSalaryPaymentDay > 31) setFormSalaryPaymentDay(1);
                    }}
                    className={`py-2 px-2.5 rounded-xl border text-center transition-all cursor-pointer ${
                      formSalaryType === 'MONTHLY'
                        ? 'bg-[#24242C] border-purple-400 text-purple-300 font-black shadow-md'
                        : 'bg-[#121214] border-[#2C2C34] text-[#8E8E98] hover:text-white'
                    }`}
                  >
                    <div className="text-xs">Aylık Maaş</div>
                    <div className="text-[10px] text-[#6E6E78]">Ayda 1 Kez</div>
                  </button>

                  <button
                    type="button"
                    onClick={() => {
                      setFormSalaryType('WEEKLY');
                      if (formSalaryPaymentDay < 1 || formSalaryPaymentDay > 7) setFormSalaryPaymentDay(5); // Cuma default
                    }}
                    className={`py-2 px-2.5 rounded-xl border text-center transition-all cursor-pointer ${
                      formSalaryType === 'WEEKLY'
                        ? 'bg-[#24242C] border-amber-400 text-amber-300 font-black shadow-md'
                        : 'bg-[#121214] border-[#2C2C34] text-[#8E8E98] hover:text-white'
                    }`}
                  >
                    <div className="text-xs">Haftalık</div>
                    <div className="text-[10px] text-[#6E6E78]">Haftada 1 Gün</div>
                  </button>

                  <button
                    type="button"
                    onClick={() => {
                      setFormSalaryType('DAILY');
                      setFormSalaryPaymentDay(0);
                    }}
                    className={`py-2 px-2.5 rounded-xl border text-center transition-all cursor-pointer ${
                      formSalaryType === 'DAILY'
                        ? 'bg-[#24242C] border-emerald-400 text-emerald-300 font-black shadow-md'
                        : 'bg-[#121214] border-[#2C2C34] text-[#8E8E98] hover:text-white'
                    }`}
                  >
                    <div className="text-xs">Günlük</div>
                    <div className="text-[10px] text-[#6E6E78]">Yevmiye</div>
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="text-xs font-bold text-[#8E8E98]">
                    {formSalaryType === 'DAILY'
                      ? 'Günlük Yevmiye (₺)'
                      : formSalaryType === 'WEEKLY'
                      ? 'Haftalık Ücret (₺)'
                  ...
                  <input
                    ...
                    placeholder={formSalaryType === 'DAILY' ? '1200.00' : formSalaryType === 'WEEKLY' ? '7000.00' : '25000.00'}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        ...
                        <input
                          ...
                          className="w-full p-2.5 bg-[#121214] border border-[#2C2C34] rounded-2xl text-xs font-mono font-bold text-purple-300 focus:border-[#F5C877] focus:outline-none"
                        />
                  ...
                  {formSalaryType === 'WEEKLY' && (
                    <>
                      <label className="text-xs font-bold text-[#8E8E98]">Haftalık Ödeme Günü</label>
                      <select
                        value={formSalaryPaymentDay}
                        onChange={(e) => setFormSalaryPaymentDay(Number(e.target.value) || 5)}
                        className="w-full mt-1 p-2.5 bg-[#121214] border border-[#2C2C34] rounded-2xl text-xs font-bold text-amber-300 focus:border-[#F5C877] focus:outline-none"
                      >
                  ...
                  {formSalaryType === 'DAILY' && (
                    <>
                      <label className="text-xs font-bold text-[#8E8E98]">Tahakkuk Periyodu</label>
                      <div className="mt-1 p-2 bg-[#121214] border border-emerald-500/30 rounded-2xl text-[11px] font-bold text-emerald-400">
                        Her Gün (Günlük Yevmiye)
                      </div>
                    </>
                  )}
                </div>
