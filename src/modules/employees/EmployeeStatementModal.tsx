...
import { Employee, EmployeePayment, dataService, WEEKDAY_LABELS } from '../../services/dataService';
          ...
          <div>
            ...
            <p className="text-xs text-[#8E8E98] mt-0.5">
              {employee.salaryType === 'DAILY'
                ...
                ? 'Haftalık Ücret: '
                : 'Aylık Maaş: '}
              <strong className="text-white">{employee.salary.toLocaleString('tr-TR')} ₺</strong>
              {employee.salaryType === 'WEEKLY' && (
                <span className="text-amber-400 font-bold ml-1">(Her {WEEKDAY_LABELS[employee.salaryPaymentDay || 5]})</span>
              )}
              {(employee.salaryType === 'MONTHLY' || !employee.salaryType) && (
                <span className="text-purple-400 font-bold ml-1">(Ayın {employee.salaryPaymentDay || 1}'i)</span>
              )}
              {employee.salaryType === 'DAILY' && (
                <span className="text-emerald-400 font-bold ml-1">(Günlük Yevmiye)</span>
              )}
              {' | '}Tel: {employee.phone || '-'}
