    ...
    <div className="h-screen w-screen bg-[#1C1C20] flex flex-col items-center justify-between p-6 select-none relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-[#141416]/90 border border-[#2C2C34] rounded-3xl p-8 shadow-2xl backdrop-blur-xl z-10 animate-fadeIn">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {onSwitchToWaiterMode && (
            <div className="pt-2 border-t border-[#2C2C34]">
              <button
                type="button"
                onClick={onSwitchToWaiterMode}
                className="w-full py-2.5 bg-[#1C1C20] hover:bg-[#25252B] border border-[#383844] text-[#F5C877] hover:text-white font-bold rounded-2xl text-xs flex items-center justify-center gap-2 transition-colors cursor-pointer"
              >
                <span>📱 Garson Mobil Terminali (PWA / Telefon)</span>
              </button>
            </div>
          )}
        </form>
