...
import { onlinePlatformService } from '../../services/onlinePlatformService';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, mobileOpen, onClose }) => {
  ...
  const [hasActiveOnlinePlatforms, setHasActiveOnlinePlatforms] = useState<boolean>(() => onlinePlatformService.isAnyPlatformEnabled());

  useEffect(() => {
    const checkActive = () => {
      setHasActiveOnlinePlatforms(onlinePlatformService.isAnyPlatformEnabled());
    };
    checkActive();
  ...
  const posMenuItems = [
    ...
    ...(hasActiveOnlinePlatforms ? [{ id: 'online-orders', label: '🛵 Yemek Platformları', icon: Bike }] : []),
  ];
    ...
    <aside className="w-72 bg-[#161619] text-[#E4E4E8] flex flex-col h-full border-r border-[#2C2C34] select-none shadow-2xl z-20 flex-shrink-0">
      ...
      <div className="p-3.5 border-t border-[#2C2C34] bg-[#121214] text-[10px] text-[#8E8E98] flex items-center justify-between">
        <div>
          ...
          <div>RYMedya Tüm Hakları Saklıdır
          </div>
        </div>
