...
class RestaurantDataService {
  ...
  public addWaiter(waiter: Partial<WaiterConfig> & { name: string; pin: string }): WaiterConfig {
    ...
    const generatedMac = (waiter.macAddress || '').trim().toUpperCase() || this.generateMacAddress();
    ...
    const newW: WaiterConfig = {
      ...
      deviceUuid: waiter.deviceUuid || `UUID-${generatedMac.replace(/[^A-Z0-9]/g, '')}`,
      deviceName: waiter.deviceName?.trim() || 'Telefon (Eşleşti)',
      status: waiter.status || 'APPROVED',
