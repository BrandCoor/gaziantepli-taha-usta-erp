...
export const exportService = {
  ...
  exportSingleEmployeeStatementPdf(employee: Employee) {
    ...
    const sTypeLabel = employee.salaryType === 'DAILY' ? 'Günlük Yevmiye' : employee.salaryType === 'WEEKLY' ? 'Haftalık Ücret' : 'Aylık Maaş';
    const sPeriodInfo = employee.salaryType === 'DAILY' ? 'Her Gün' : employee.salaryType === 'WEEKLY' ? `Her Hafta (${employee.salaryPaymentDay || 5}. gün)` : `Her Ayın ${employee.salaryPaymentDay || 1}'i`;
    ...
    const html = `
      ...
      <body>
        ...
        <div class="info-card"><div><div>Personel: <strong>${employee.fullName}</strong></div><div>Görev: <strong>${employee.position || '-'}</strong> | Tel: ${employee.phone || '-'}</div>${employee.iban ? `<div>IBAN: <code>${employee.iban}</code></div>` : ''}</div><div style="text-align:right;"><div style="color:#64748b;font-size:11px;">Giriş: ${formatDate(employee.startDate)}</div><div>${sTypeLabel}: <strong>${formatCurrency(employee.salary)}</strong> (${sPeriodInfo})</div><div style="margin-top:4px;font-size:13px;font-weight:800;" class="${employee.balance > 0 ? 'text-rose' : 'text-emerald'}">Kalan: ${formatCurrency(employee.balance)}</div></div></div>
  ...
  async exportEmployeesExcel(employees: Employee[], totalBalance: number, dateRange?: DateRange) {
    ...
    worksheet.getRow(2).font = { name: 'Segoe UI', size: 10, italic: true, color: { argb: 'FF64748B' } };
    worksheet.addRow([]);
    const headerRow = worksheet.addRow(['Personel Ad Soyad', 'Görevi', 'Ödeme Tipi', 'Telefon', 'IBAN', 'Maaş / Ücret (TL)', 'Kalan Borç (TL)']);
    worksheet.columns = [{ key: 'fullName', width: 28 }, { key: 'position', width: 22 }, { key: 'salaryType', width: 18 }, { key: 'phone', width: 18 }, { key: 'iban', width: 34 }, { key: 'salary', width: 22 }, { key: 'balance', width: 26 }];
    headerRow.font = { name: 'Segoe UI', size: 11, bold: true, color: { argb: 'FFFFFFFF' } };
    headerRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E293B' } };
    employees.forEach((e) => {
      const sType = e.salaryType === 'DAILY' ? 'Günlük Yevmiye' : e.salaryType === 'WEEKLY' ? 'Haftalık' : 'Aylık';
      const row = worksheet.addRow({ fullName: e.fullName, position: e.position || '-', salaryType: sType, phone: e.phone || '-', iban: e.iban || '-', salary: e.salary, balance: e.balance });
      row.getCell(6).numFmt = '#,##0.00 "₺"';
      row.getCell(7).numFmt = '#,##0.00 "₺"';
      if (e.balance > 0) row.getCell(7).font = { color: { argb: 'FFE11D48' }, bold: true };
    });
  ...
  exportEmployeesPdf(employees: Employee[], totalBalance: number, dateRange?: DateRange) {
    ...
    const html = `
      ...
      <body>
        ...
        <table><thead><tr><th>#</th><th>Personel</th><th>Görev</th><th>Ödeme Sıklığı</th><th>Maaş / Ücret</th><th style="text-align:right;">Kalan Borç</th></tr></thead>
        <tbody>${employees.map((e, i) => {
          const sType = e.salaryType === 'DAILY' ? 'Günlük Yevmiye' : e.salaryType === 'WEEKLY' ? `Haftalık (${e.salaryPaymentDay || 5}. gün)` : `Aylık (Ayın ${e.salaryPaymentDay || 1}'i)`;
          return `<tr><td>${i+1}</td><td><b>${e.fullName}</b></td><td>${e.position||'-'}</td><td><span style="font-size:11px;color:#4f46e5;font-weight:bold;">${sType}</span></td><td>${formatCurrency(e.salary)}</td><td style="text-align:right;color:#e11d48;font-weight:bold;">${formatCurrency(e.balance)}</td></tr>`;
        }).join('')}</tbody></table>
