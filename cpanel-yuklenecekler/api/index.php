...
header('Access-Control-Allow-Headers: Content-Type, Authorization, X-Device-UUID, X-Device-Token, X-Requested-With');
...
if ($action === 'verify_waiter_pin' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    // ====================================================================
    // GARSON GİRİŞİ: yalnızca PIN ile. PIN'ler benzersiz olduğu için garson
    // doğrudan PIN'den tanınır; ayrıca personel ID veya cihaz eşleştirmesi
    // istenmez. 4 haneli PIN tek başına zayıf olduğundan cihaz bazlı deneme
    // sınırı uygulanır (aksi halde 10.000 kombinasyon denenebilirdi).
    // ====================================================================
    $input = json_decode(file_get_contents('php://input'), true);
    $pin = preg_replace('/\D/', '', (string)($input['pin'] ?? ''));
    $deviceUuid = trim($input['device_uuid'] ?? ($_SERVER['HTTP_X_DEVICE_UUID'] ?? ''));
    $throttleKey = $deviceUuid !== '' ? $deviceUuid : ('ip-' . ($_SERVER['REMOTE_ADDR'] ?? 'bilinmeyen'));
    ...
    if ($useMysql) {
        // Deneme sayacı tablosu (yoksa oluşturulur)
        try {
        ...
        if ($w) {
            ...
            // Cihaz kaydı: eşleştirme şartı değil, hangi telefonun giriş yaptığını
            // görebilmek ve gerekirse kasadan erişimi kesebilmek için tutulur.
            if ($deviceUuid !== '') {
                try {
                    $pdo->prepare("UPDATE `personeller` SET `device_uuid` = ?, `device_paired_at` = NOW() WHERE `id` = ?")->execute([$deviceUuid, $w['id']]);
        ...
        // 3) Hatalı deneme: 5 denemeden sonra 10 dakika kilit
        try {
        ...
        foreach (($db['employees'] ?? []) as $e) {
            ...
            $empPin = preg_replace('/\D/', '', (string)($e['pin'] ?? ''));
            if ($empPin !== '' && $empPin === $pin) $matches[] = $e;
        }
